...
export class PaliNode {
  private parent = $state<PaliNode | undefined>(undefined);
  children: PaliNode[] = $state([]);
  description = $state('');
  kind = $state<'paragraph' | 'section' | 'verse' | 'word'>('section');
  name = $state('');
  pali = $state('');
  thai = $state('');
...
export const tokenizePaliText = (paliText: string) =>
  pipe(
    ...
    A.filter(G.isObject),
    F.tap((arr) => console.log(paliText, arr))
  );
