<script lang="ts">
  import type { PaliNode, tokenizePaliText } from '$lib/patimokkha.svelte';
...
{#if node.kind === 'section'}
  <div class="">
    <div class="sticky top-0 z-30 border-b bg-red-200 p-2 text-sm">{node.name}</div>
...
{:else if node.kind === 'verse'}
  <div class="relative text-left {node.pali.endsWith('.') ? 'mb-6' : ''}">
